...
const FileUploadModal = ({ isOpen, onClose }) => {
    ...
    const validateFile = async (file) => {
        ...
        if (validTypes.includes(file.type)) {
            ...
            try {
                const res = await axios.post(`${baseUrl}resume/extract`, formData, {
                    headers: {
                        'Authorization': `${authToken}`,
                        'Content-Type': 'multipart/form-data'
                    }
                });
                console.log(res.data.data.resumeId);
                localStorage.setItem('resumeId', res.data.data.resumeId);
            ...
            <DialogContent className="sm:max-w-md max-h-[75vh] overflow-y-auto">
                ...
                <div className="grid gap-6 py-4">
                    <div
                        ...
                        onDrop={handleDrop}
                        className={`flex flex-col items-center justify-center border-2 border-dashed rounded-md p-6 transition-all duration-200 ${isDragging
